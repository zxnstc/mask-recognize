...
# -----------------------------#
#   mtcnn的第二段
#   精修框：对没有人脸的框剔除，有人脸的修正
# -----------------------------#
def create_Rnet(weight_path):
    # 把从Pnet中提取出来的图像进行resize成24×24
...
class mtcnn():
    ...
    def detectFace(self, img, threshold):
        ...
        for i in range(len(scales)):
            # ------------------------------------------------------------------#
            #   在上面对图像金字塔循环的时候就把batch_size维度给去掉了
            # ------------------------------------------------------------------#
            # 有人脸的概率
            cls_prob = out[i][0][:, :, 1]
            # 网格点框的位置
